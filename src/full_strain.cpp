		...
		strain *mother_strain, double inputBeta)
{
	...
	else
	{
...
#ifdef IMMUN_RAMP
	    else if (params->time >  params->immun_delay_time +  params->immun_ramp_time)
	    {
		// ramp in E0 if early in sim
		for (int i=0; i < MAX_COMPARTMENTS; i++)
		{
		    double drawn = gsl_ran_weibull(params->ur,params->cd8_mean,params->cd8_k);
		    cd8Cells[i] = (int)(drawn);
		}
	    }
	    else
	    {
		for (int i=0; i < MAX_COMPARTMENTS; i++)
		{
		    double cd80 = gsl_ran_weibull(params->ur,params->cd8_0,params->cd8_k);
		    if (params->time < params->immun_delay_time)
			cd8Cells[i] = cd80;
		    else
		    {
			double factor = MIN((params->time-params->immun_delay_time),params->immun_ramp_time)/params->immun_ramp_time;
			double drawn = gsl_ran_weibull(params->ur,params->cd8_mean,params->cd8_k);

			if (drawn < cd80)
			    drawn = cd80;
			cd8Cells[i] = cd80 + factor *(drawn - cd80);
		    }
		    if (cd8Cells[i] < MIN_STRAIN_CD8S)
			cd8Cells[i]=MIN_STRAIN_CD8S;
		}
	    }
#else
	    else
...
double strain::get_ic50(settings *params, double time)
{
...
#ifdef IMMUN_RAMP
    if (time <= params->immun_delay_time)
	return strainIC50_0;

    else if (time >  params->immun_delay_time +  params->immun_ramp_time)
#endif
	return strainIC50;
    ...
    double this_ic50 = (1-factor)* (strainIC50_0 - strainIC50) +  strainIC50;
    return this_ic50;
#endif
}
