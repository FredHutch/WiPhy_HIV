...
//   the following sections are skipped for "junk"
	...
	if (num_act_cells > 0)
	{
	    ...
	    if (params->dA_density > 0)
		natural = gsl_ran_poisson (params->ur, 
		    dA_immun*pow(left_over,params->dA_density)*params->dt);
	    else
		natural = gsl_ran_poisson (params->ur, 
...
void read_input_file(char *inp_file, settings *params)
{
	...
	*valuep = '\0';
    ...
    CHECK_FOR_REAL("dA_density",dA_density );
...
double ScoreFunction(settings *params)
{
	...
	{
	    if (params->ART_stop > 0 && time >= params->ART_stop && params->start_time == 0)
	    {
		params->start_time = time;
		fprintf(stdout,"Start time reset to %lf\n",time);
		peak_vl = 0;
		peak_time = 0;
		nadir_vl = 0;
		nadir_time = 0;
	    ...
	    if (params->ART_start > 0 && (time-params->start_time) >= params->ART_start && 
		(time-params->start_time)<=params->ART_start+params->dt)
	    {
		    fprintf(stdout,"Run %d, real-ART started at t=%lf...\n",
			runnum+1,time);
	    }
	    if (params->ART_stop > 0 && (time-params->start_time) >= params->ART_stop && 
		(time-params->start_time)<=params->ART_stop+params->dt)
	    {
		    fprintf(stdout,"Run %d, real-ART stopped at t=%lf...\n",
			runnum+1,time);
	    }
		...
		{
		    char err_msg[200];
		    sprintf(err_msg,"Peak is too late, stopping run at t=%3.2lf (start_time=%lf)...\n",
		...
		nadir_vl = x[4]+x[5];
		log_nadir_drop = 0;
		peak_time = time-params->start_time;
		nadir_time = time-params->start_time;
		num_set_pts=0;
		set_pt_err=0;
		set_pt_p=0;
	    }
	    else if (peak_vl > 0 && x[4]+x[5] < nadir_vl && 
		(time-params->start_time) - peak_time < 25. && num_set_pts==0)
	    {
		nadir_vl = x[4]+x[5];
		if (nadir_vl > 0)
		...
		nadir_time = time-params->start_time;
		last_set_time = time-params->start_time;
	    }
	    else if (peak_time > 0 && nadir_time > peak_time + 5 && (time-params->start_time) - last_set_time > 1.)
	    {
