...
my $low_mean_to_sd=0.1;
my $high_mean_to_sd=1.0;
my $range_mean_to_sd= $high_mean_to_sd-$low_mean_to_sd;

my $low_dA_IC50=1;
my $high_dA_IC50=5;
my $range_dA_IC50= $high_dA_IC50-$low_dA_IC50;

my $low_dA_scale=1;
my $high_dA_scale=5;
my $range_dA_scale= $high_dA_scale-$low_dA_scale;
...
while ( $count > 0 ){
    ...
    my $new_dA_IC50= $range_dA_IC50*rand() + $low_dA_IC50;
    my $new_dA_scale= $range_dA_scale*rand() + $low_dA_scale;
    ...
    printf TSTFILE ("dA_ic50_0 %g\n",$new_dA_IC50);
    printf TSTFILE ("dA_scale %g\n",$new_dA_scale);
